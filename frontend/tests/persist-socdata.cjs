...
let b=boot();
ck("schema_version recorded", b.soc.schemaVersion()===4, String(b.soc.schemaVersion()));
...
b.soc.upsertIncident({ref:"INC-2041",title:"Credential attack",severity:"high",status:"investigating",
  ...
  riskScore:78,minutesAgo:14,updatedMinutesAgo:2,techniqueIds:["T1110"],
  assets:[{name:"WS-042"}],timeline:[{at:"t0",what:"first failure"}],
  eventsCorrelated:11,isolationSimulated:false});
b.db.close(); b=boot();
...
ck("riskScore preserved", inc.riskScore===78, String(inc.riskScore));
ck("techniqueIds preserved", inc.techniqueIds[0]==="T1110");
ck("assets preserved", inc.assets.length===1);
ck("timeline preserved", inc.timeline.length===1);
ck("eventsCorrelated preserved", inc.eventsCorrelated===11);
ck("isolationSimulated preserved", inc.isolationSimulated===false);
...
b.soc.upsertInvestigation({alertRef:"ALT-10492",status:"investigating",assignedTo:"J. Mehta",
  ...
  investigationId:"INV-771",openedMinutesAgo:22,updatedMinutesAgo:3});
b.db.close(); b=boot();
...
ck("investigationId restored not regenerated", iv.investigationId==="INV-771", String(iv.investigationId));
ck("openedMinutesAgo restored", iv.openedMinutesAgo===22, String(iv.openedMinutesAgo));
ck("updatedMinutesAgo restored", iv.updatedMinutesAgo===3);
